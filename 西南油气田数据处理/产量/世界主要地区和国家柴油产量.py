...
df = pd.read_excel(r'C:\Users\Administrator\Desktop\西南油气田数据\数据整理\世界主要地区和国家柴油产量.xlsx', engine='openpyxl', header=2)
...
result['柴油产量（万吨）'] = pd.to_numeric(result['柴油产量（万吨）'], errors='coerce')
print(result)
os.makedirs('../处理后数据', exist_ok=True)
output_path = '../处理后数据/产量/世界主要地区和国家柴油产量处理后数据.xlsx'
